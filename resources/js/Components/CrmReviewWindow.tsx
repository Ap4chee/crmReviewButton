...
export default function CrmReviewModal({ clientWebsiteUrl = "", onReviewSaved }: CrmReviewModalProps) {
  ...
  const [editingRectangle, setEditingRectangle] = useState<string | null>(null)

  const CRM_BACKEND_API_ENDPOINT = "http://127.0.0.1:8000/api/reviews"
  ...
  const sendMessageToIframe = useCallback(
    (message: any) => {
      if (iframeRef.current?.contentWindow && clientWebsiteUrl) {
        iframeRef.current.contentWindow.postMessage(message, clientWebsiteUrl)
      }
    },
    [clientWebsiteUrl],
  )
    ...
    async (event: MessageEvent) => {
      ...
      if (event.data?.type === "reviewData") {
        if (isSavingRef.current === false) {
          console.warn("Ignoring reviewData event - not in saving state")
          return
        }

        console.log("Review data received:", event.data.payload)

        try {
          const response = await fetch(CRM_BACKEND_API_ENDPOINT, {
            method: "POST",
          ...
          if (response.ok) {
            ...
            console.error("Failed to save review:", response.status, await response.text())
          }
        } catch (error) {
        ...
        console.log("ℹ️ Review mode exited on client website.")
        resetSavingState()
        handleCloseModal()
      }
    },
  ...
  const handleUpdateRectangleComment = (id: string, comment: string) => {
    sendMessageToIframe({ type: "updateRectangleComment", payload: { id, comment } })
    setEditingRectangle(null)
  }
  ...
  return (
    <>
      ...
      <Button onClick={handleOpenModal} disabled={!clientWebsiteUrl} className="flex items-center gap-2">
                  ...
                  <CardContent className="space-y-3">
                    <Button
                      ...
                      className={`w-full justify-start ${
                        isDrawingMode ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-transparent"
                      }`}
                      disabled={isSaving}
                    >
                      <Pencil className="w-4 h-4 mr-2" />
                      {isDrawingMode ? "Zaznaczanie WŁĄCZONE" : "Włącz/Wyłącz zaznaczanie"}
                    </Button>
                    <Button
                      onClick={() => sendMessageToIframe({ type: "undoLast" })}
                      variant="outline"
                  ...
                  <CardContent className="space-y-2 max-h-100 overflow-y-auto">
                    {rectangles.length === 0 ? (
                      <p className="text-sm text-gray-500 text-center py-4">
                        Brak uwag. Włącz tryb rysowania, zaznacz co chciałbyś zmienić i napisz na co.
                      </p>
                    ) : (
                      rectangles.map((rect) => (
                        <div key={rect.id} className="border rounded-lg p-3 bg-white">
                          ...
                          {editingRectangle === rect.id ? (
                            <div className="space-y-2">
                              <Input
                                defaultValue={rect.comment}
                                placeholder="Dodaj komentarz..."
                                className="text-xs"
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    handleUpdateRectangleComment(rect.id, e.currentTarget.value)
                                  } else if (e.key === "Escape") {
                                    setEditingRectangle(null)
                                  }
                                }}
                                onBlur={(e) => {
                                  handleUpdateRectangleComment(rect.id, e.target.value)
                                }}
                                autoFocus
                              />
                            </div>
                            ...
                            <div
                              ...
                              onClick={() => setEditingRectangle(rect.id)}
                            >
